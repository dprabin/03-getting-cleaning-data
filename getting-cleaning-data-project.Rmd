...
data1<-cbind(newcol,reduced)[,-c(2,3)]
#find mean by newcol
newdata<-data1 %>% group_by(newcol) %>% summarise_each(funs(mean))
#now write this data to a file
write.table(newdata,file="tidydata_mean_subject-activities.txt",row.name=FALSE)

#compare this to original data
dim(data)
dim(reduced)
#cleaned and reduced data
dim(newdata)
###
#a<-length(unique(reduced$subject))
#b<-length(unique(reduced$activity))
#a*b=180
```